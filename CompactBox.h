...
void serialESPInitialize(){
    ...
    sendToESP("AT+CWJAP=\"Carvalho\",\"02724913221\"", 6000);
}
...
String sendDataa(String IDdispositivo, float valorSensor){
  sendToESP("AT+CIPSTART=1,\"TCP\",\"www.compactbox.tk\",80",2000);
  
  String GETrequest1 =  "GET /compactbox/getDispositivo.php";       // 34
  String GETrequest2 =  " HTTP/1.0\r\n";                            // 13
         GETrequest2 += "Host: \"www.compactbox.tk\"\r\n";          // 31
         GETrequest2 += "Connection: close";                        // 17
  
  sendToESP("AT+CIPSEND=1,95", 300);
  sendToESP(GETrequest1      , 300);
  sendToESP(GETrequest2      , 300);
